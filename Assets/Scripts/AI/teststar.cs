...
class Pathfinder : MonoBehaviour
    {
		...
		public Pathfinder(Board map,Shape active ,int rotation)
        {
			width = map.m_width;
			height = map.m_height;
			InitializeSearchNodes(map,active,rotation);
		Debug.Log ("he;;ooo");
        }
		...
		private void InitializeSearchNodes(Board map,Shape active, int rotation)
        {
            ...
            for(int x = 0; x < width; x++)
            {
                for (int y = 0; y < height; y++)
                {
					...
					node.Walkable = map.IsValidPosition(active, new Cell(x, y,map),rotation);
				Debug.Log ("hehehf");
                    if (node.Walkable)
                    {
